...
- Definir procesos *forward* (SDEs: VP, VE, sub-VP, CLD) que destruyen los datos hacia ruido.
- Entrenar la red por *denoising score matching* (DSM) para una SDE dada.
- Integrar el proceso reverso con distintos *samplers* (pendiente) para generar muestras.
...
independientes forman una matriz de experimentos **4×4 = 16 celdas**:

- **Eje 1 — Proceso forward (SDE):** VP, VE, sub-VP, CLD. Cambiarlo **obliga a reentrenar** (cada
...
(Song et al. 2021 como referencia ancla) y validación cuantitativa (Monte Carlo, score analítico).
